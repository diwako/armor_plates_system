...

{
	...
	if ( _distance > GVAR(showDownedUnitIndicatorRange) && 
	{ GVAR(visibleBleedoutTimer) == 0 || {_distance > GVAR(bleedoutTimerRange) || 
	{ !(_isMedic || GVAR(visibleBleedoutTimer) == 2 ) } } } ) then {continue};
