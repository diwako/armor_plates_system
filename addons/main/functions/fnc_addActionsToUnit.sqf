...
}, [], GVAR(medicReviveTime), 15, false, false, false];

_arr call BIS_fnc_holdActionAdd;
private _arr2 = +_arr;
_arr2 set [4, format ["alive _originalTarget && { _originalTarget isNotEqualTo _this && {(lifeState _originalTarget) == 'INCAPACITATED' && {!(_this getUnitTrait 'Medic') && {(_originalTarget distance _this) < 4 && {[_this] call %1 > 0}}}}}", QFUNC(hasHealItems)]];
