...
_arr3 call BIS_fnc_holdActionAdd;
    ...
    private _id2 = _unit addAction ["<img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_revive_ca.paa' size='1.8' shadow=2 />", {
        params ["", "_caller"];
        private _isProne = stance _caller == "PRONE";
        private _medicAnim = ["AinvPknlMstpSlayW[wpn]Dnon_medicOther", "AinvPpneMstpSlayW[wpn]Dnon_medicOther"] select _isProne;
        private _wpn = ["non", "rfl", "lnr", "pst"] param [["", primaryWeapon _caller, secondaryWeapon _caller, handgunWeapon _caller] find currentWeapon _caller, "non"];
    ...
    format ["_this getUnitTrait 'Medic' && {alive _originalTarget && {([_this] call %1) > 0}}", QFUNC(hasInjector)],
    2.5];
    _unit setUserActionText [_id2, format [LLSTRING(useInjector), name _unit], "<img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_revive_ca.paa' size='1.8' shadow=2 />"];