...
WBK_CreateDamage = {
    ...
    [_unit, _damage * 2, ["body", "head"] select (animationState _killer in IMS_HeadDamageAnimations || gestureState _killer in IMS_HeadDamageAnimations), _killer, "", GVAR(ignoreArmor)] call EFUNC(main,receiveDamage);

    if (_unit isEqualTo (call CBA_fnc_currentUnit)) then {
        [40] call BIS_fnc_bloodEffect;
        enableCamShake true;
        addCamShake [15, 1, 40];
        if (missionNamespace getVariable ["SFX_EnableBreathing_Hurt", false]) then {
